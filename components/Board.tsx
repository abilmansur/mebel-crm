  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-2">
      {STAGES.map((stage) => {
        ...
        return (
          <div key={stage.id} className="min-w-[190px] flex-none">
            <div className="flex justify-between text-sm font-medium text-ink/60 px-1 pb-2">
              <span>{t(`stage.${stage.id}`)}</span>
              <span>{list.length}</span>
            </div>
            <div
              // Большая min-height даёт много "свободного воздуха" под карточками —
              // так промах на несколько пикселей мимо последней карточки всё равно
              // засчитывается как drop в нужный столбец, а не мимо него.
              className={`min-h-[320px] rounded-lg transition-colors p-1 -m-1 ${
                dragOverStage === stage.id ? "bg-oak/10 ring-2 ring-oak/30" : ""
              }`}
              onDragEnter={(e) => {
                e.preventDefault();
              ...
              onDragLeave={(e) => {
                // Игнорируем "ложный" dragLeave при переходе между дочерними элементами внутри столбца
              ...
              onDrop={(e) => {
                ...
                if (orderId) onStatusChange(orderId, stage.id);
              }}
            >
              {list.map((order) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-surface border border-line rounded-xl p-3 mb-2.5 cursor-grab active:cursor-grabbing hover:border-oak/50 transition-opacity ${
              ...
              <button
                onClick={() => onAddAtStage(stage.id)}
                className="w-full border border-dashed border-line rounded-lg py-2 text-xs text-ink/40 hover:text-oak hover:border-oak/40 transition-colors"
              >
                + {t("nav.newOrder")}
              </button>
            </div>
